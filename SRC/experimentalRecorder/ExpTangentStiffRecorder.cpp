...
void* OPF_ExpTangentStiffRecorder()
{
    ...
    while (OPS_GetNumRemainingInputArgs() > 0) {
        ...
        else if (strcmp(option, "-tangStif") == 0 ||
            strcmp(option, "-tangStiff") == 0) {
            while (OPS_GetNumRemainingInputArgs() > 0) {
                int tag;
                numdata = 1;
                ...
                tangStifTags[numTangStif++] = tag;
            }
        }
        else if (strcmp(option, "-tangStifRange") == 0 ||
            strcmp(option, "-tangStiffRange") == 0) {
            int start = 0, end = 0;
            if (OPS_GetNumRemainingInputArgs() > 0) {
                numdata = 1;
                if (OPS_GetIntInput(&numdata, &start) < 0) {
                    opserr << "WARNING: failed to read start tangent stiff tag\n";
                    return 0;
                }
            }
            if (OPS_GetNumRemainingInputArgs() > 0) {
                ...
                if (OPS_GetIntInput(&numdata, &end) < 0) {
                    opserr << "WARNING: failed to read end tangent stiff tag\n";
                    return 0;
                }
            }
            if (start > end) {
    ...
    for (int i = 0; i < numTangStif; i++) {
        theTangStiffs[i] = OPF_getExperimentalTangentStiff(tangStifTags(i));
    ...
    ExpTangentStiffRecorder* recorder = new ExpTangentStiffRecorder(numTangStif,
        theTangStiffs, data, nargrem, echoTimeFlag, *theOutputStream, deltaT);
