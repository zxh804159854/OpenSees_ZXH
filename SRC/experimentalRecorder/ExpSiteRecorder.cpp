...
void* OPF_ExpSiteRecorder()
{
    ...
    while (OPS_GetNumRemainingInputArgs() > 0) {
        ...
        else if (strcmp(option, "-siteRange") == 0) {
            int start = 0, end = 0;
            if (OPS_GetNumRemainingInputArgs() > 0) {
                numdata = 1;
                if (OPS_GetIntInput(&numdata, &start) < 0) {
                    opserr << "WARNING: failed to read start site tag\n";
                    return 0;
                }
            }
            if (OPS_GetNumRemainingInputArgs() > 0) {
                ...
                if (OPS_GetIntInput(&numdata, &end) < 0) {
                    opserr << "WARNING: failed to read end site tag\n";
                    return 0;
                }
            }
            if (start > end) {
    ...
    ExpSiteRecorder* recorder = new ExpSiteRecorder(numSites, theSites,
