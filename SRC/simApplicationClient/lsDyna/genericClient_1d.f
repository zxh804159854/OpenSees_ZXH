...
** the OpenFresco middle-tier server through a tcp/ip connection.
c
...
c
               call senddata(socketID, sizeInt, iData, 11, stat)
            endif
         else
...
c
         ...
         hsv(i,3) = hsv(i,3) + dy1(i)
         hsv(i,4) = hsv(i,4) + dx2(i)
         hsv(i,5) = hsv(i,5) + dy2(i)
c
...
c
         ...
         enddo
c
         call senddata(socketID, sizeDouble, sData, sizeSendData, stat)
c
...
c
         ...
         call senddata(socketID, sizeDouble, sData, sizeSendData, stat)
c
         call recvdata(socketID, sizeDouble, rData, sizeSendData, stat)
c
         do j = 1, 2
            force(i,j)   = rData(j)
            force(i,j+6) = rData(j+2)
         enddo
      enddo
