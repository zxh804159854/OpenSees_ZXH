...
// Revision: A
//
// Description: This file contains the class definition for SimFEAdapter.
// SimFEAdapter is a Simulink interface for communicating with an adapter
// element of an other FE-software over a single TCP/IP connection.

#define S_FUNCTION_NAME SFun_SimFEAdapter
...
// SimFEAdapter parameters
#define ipAddr(S)   ssGetSFcnParam(S,0)    // ip address of adpater element
#define ipPort(S)   ssGetSFcnParam(S,1)    // ip port of adapter element
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    // allocate memory for past ctrl disp vector
    ssSetDWorkWidth(S, 4, 256);
...
static void mdlStart(SimStruct *S)
{
    ...
    // get parameters
    int_T sizeCtrlDisp = ssGetInputPortWidth(S,0);
    ...
    real_T *ctrlDispPast = (real_T*)ssGetDWork(S,4);
    
    // initialize socketID and dataSize
    socketID[0] = -1;
    dataSize[0] = (int_T)mxGetScalar(dataSize(S));
    dataSize[0] = (1+sizeCtrlDisp>dataSize[0]) ? 1+sizeCtrlDisp : dataSize[0];
    ...
    // resize and initialize past ctrl disp vector
    ssSetDWorkWidth(S, 4, sizeCtrlDisp);
    ...
    mxFree(ipAddr);
    if (socketID[0] < 0)  {
        ssSetErrorStatus(S,"Failed to setup connection with adapter element");
        return;
    }
    
    // set the data size for the server
    // sizeCtrl
    iData[0] = sizeCtrlDisp;    // disp
    iData[1] = 0;               // vel
    iData[2] = 0;               // accel
    iData[3] = 0;               // force
    iData[4] = 0;               // time
...
{
    InputRealPtrsType ctrlDisp = ssGetInputPortRealSignalPtrs(S,0);
    real_T *daqDisp  = ssGetOutputPortRealSignal(S,0);
    real_T *daqForce = ssGetOutputPortRealSignal(S,1);
    
    int_T i, ierr, nleft, dataTypeSize, hasChanged;
    char_T *gMsg;
    ...
    // check if ctrl displacments have changed
    hasChanged = 0;
    for (i=0; i<sizeCtrlDisp; i++)  {
        if ((*ctrlDisp[i]) != ctrlDispPast[i])  {
            hasChanged = 1;
            break;
        }
    }
    ...
    if (hasChanged==1)  {
        // send ctrl displacements
        sData[0] = 3;
        for (i=0; i<sizeCtrlDisp; i++)  {
            sData[1+i] = (*ctrlDisp[i]);
            ctrlDispPast[i] = (*ctrlDisp[i]);
        }
        gMsg = (char_T *)sData;
